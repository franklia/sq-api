...
router.get(
  '/questions/index',
  (req, res, next) => {
    Questions.find({ auth0_id: req.query.userId }).then((data) => {
      if (data.length === 0) {
        res.json(data);
      } else {
        res.questions = data;
...
router.put('/user/category/create', (req, res) => {
  ...
  const saveCategories = () => {
    ...
    const updateValues = {
      $push: { categories: { _id: ObjectId(), name: req.body.categoryName } },
    };
  ...
  Users.findOne({ auth0_id: req.body.auth0Id })
    .select('_id')
    .lean()
    .then((data) => {
...
router.put('/user/category/update', (req, res) => {
  const findBy = {
    auth0_id: req.body.auth0Id,
    'categories._id': req.body.updateOrDeleteCategoryId,
  };
...
router.get(
  ...
  (req, res, next) => {
    ...
    Questions.find({ category: req.params.category }).then((data) => {
      if (data.length === 0) {
        res.json(data);
      } else {
        next();
